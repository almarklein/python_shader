...
class PyBytecode2Bytecode:
    ...
    def _check_if_this_is_a_while_test(self, target):
        ...
        if target in jump_targets:
            if loop_info["type"] == "while" and loop_info["body_label"] == 0:
                loop_info["body_label"] = self._pointer
                self._labels[loop_info["continue_label"]] = loop_info["continue_label"]
    ...
    def _op_setup_loop(self):
        ...
        loop_info = {
            ...
            # The body_label represents the body of the loop. We can't set it yet.
            "body_label": 0,
            # Keep track whether a range has been specified (for-loops)
            "range_specified": 0,
        }
        ...
        # -> do not create the continue_label yet, otherwise a co_label will be
        #    autogenerated in _convert, and we need that label in a different place.
        #    Instead we emit it in _check_if_this_is_a_while_test()
    ...
    def _op_pop_block(self):
        ...
        if loop_info["type"] == "while":
            ...
            self.emit(op.co_load_constant, 0.0)
            self.emit(op.co_pop_top)
            self.emit(op.co_branch, loop_info["header_label"])
        else:
